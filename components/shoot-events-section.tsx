    ...
    <AnimatedSection className="py-24 sm:py-28">
      <div className="section-shell">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          ...
          <div className="hero-card hover-glow-card rounded-[32px] p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="inline-flex items-center gap-3 rounded-full border border-gold/20 bg-gold/10 px-4 py-3 text-sm text-white/85">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
          <div className="overflow-hidden rounded-[32px] border border-white/10 bg-white/10 p-4 shadow-glow">
            <AnimatePresence mode="wait">
              ...
              >
                <div
                  ref={frameRef}
                  className="relative overflow-hidden rounded-[26px] border border-white/10 bg-black/25"
                >
                  <div className="absolute inset-x-3 bottom-3 z-10 flex flex-col gap-3 sm:inset-x-auto sm:bottom-4 sm:right-4 sm:max-w-[calc(100%-2rem)]">
                    <div className="inline-flex w-fit rounded-full border border-white/15 bg-black/35 p-1 backdrop-blur-xl">
                      <button
                        type="button"
                        onClick={() => setViewMode("landscape")}
                        className={`rounded-full px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] transition sm:px-4 ${
                          viewMode === "landscape" ? "bg-gold text-black" : "text-white/72"
                        }`}
                      >
                        Landscape
                      </button>
                      <button
                        ...
                        className={`rounded-full px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] transition sm:px-4 ${
                          viewMode === "vertical" ? "bg-gold text-black" : "text-white/72"
                        }`}
                      >
                        Vertical
                      </button>
                    ...
                    <button
                      type="button"
                      className="inline-flex w-fit items-center gap-2 rounded-full border border-white/15 bg-black/35 px-3 py-2 text-xs text-white backdrop-blur-xl transition hover:bg-black/45 sm:px-4"
                  ...
                  <button
                    type="button"
                    className="absolute left-4 top-4 z-10 inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/35 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-white backdrop-blur-xl transition hover:bg-black/50"
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-5 pb-28 sm:pb-5">
                    <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/25 px-3 py-2 text-xs uppercase tracking-[0.24em] text-gold backdrop-blur-xl">
                      <PlayCircle size={14} />
                      Playable Video
                    </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {shootEventMedia.images.map((item) => (
              <article
                key={item.title}
                className="group hover-glow-card overflow-hidden rounded-[28px] border border-white/10 bg-white/10 shadow-glass"
              >
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="absolute left-4 top-4 inline-flex rounded-full border border-white/15 bg-black/20 px-3 py-2 text-xs uppercase tracking-[0.22em] text-white backdrop-blur-xl">
      ...
      <AnimatePresence>
        {showVideoModal && (
          ...
          >
            <motion.div
              className="relative w-full max-w-5xl overflow-hidden rounded-[32px] border border-white/10 bg-black"
